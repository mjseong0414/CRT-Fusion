...
def create_nuscenes_infos(root_path,
    ...
    if test:
        ...
        info_path = osp.join(root_path,
                             '{}_infos_crtfusion_test.pkl'.format(info_prefix))
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(root_path,
                             '{}_infos_crtfusion_train.pkl'.format(info_prefix))
        mmcv.dump(data, info_path)
        data['infos'] = val_nusc_infos
        info_val_path = osp.join(root_path,
                                 '{}_infos_crtfusion_val.pkl'.format(info_prefix))
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if not test:
            ##### load and save lidar seg points #####
            lidarseg_filename = osp.join(nusc.dataroot, nusc.get('lidarseg', lidar_token)['filename'])
            ...
            lidarseg_filename = 'data/nuscenes/points_seg/' + lidarseg_filename.split('/')[-1].split('.')[0] + '.npy'
            np.save(lidarseg_filename, points_seg)
            ##### load and save lidar seg points #####
        ...
        if not test:
            info['lidarseg_path'] = lidarseg_filename
        
        radar_max_sweeps = 7
        make_multisweep_radar_data(nusc,sample,radar_max_sweeps,root_path,info)
