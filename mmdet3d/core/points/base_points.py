...
class BasePoints(object):
    ...
    def rotate_radar(self, rotation, axis=None):
        """Rotate points with the given rotation matrix or angle.
        ...
        if rotation.numel() == 1:
            ...
            raise NotImplementedError
        self.tensor[:, :3] = self.tensor[:, :3] @ rot_mat_T # radar 3D
        self.tensor[:, 4:6] = self.tensor[:, 4:6] @ rot_mat_T[:2, :2] # radar velocity
    ...
    def scale_radar(self, scale_factor):
        """Scale the points with horizontal and vertical scaling factors.
