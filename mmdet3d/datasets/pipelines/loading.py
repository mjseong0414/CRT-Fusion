...
@PIPELINES.register_module()
class PointToMultiViewDepthSeg(object):
    def __init__(self, grid_config, downsample=16):
        self.downsample = downsample
        self.grid_config=grid_config

    def points2depthsegmap(self, points, points_seg_tmp, height, width, canvas=None):
        height, width = height//self.downsample, width//self.downsample
        depth_map = torch.zeros((height,width), dtype=torch.float32)
        seg_map = torch.zeros((height,width), dtype=torch.int16)
        coor = torch.round(points[:,:2]/self.downsample)
        depth = points[:,2]
        kept1 = (coor[:, 0] >= 0) & (coor[:, 0] < width) \
               & (coor[:, 1] >= 0) & (coor[:, 1] < height) \
        ...
        coor, depth = coor[kept1], depth[kept1]
        points_seg_tmp = points_seg_tmp[kept1]
        ranks = coor[:, 0] + coor[:, 1] * width
        sort = (ranks+depth/100.).argsort()
        coor, depth, ranks = coor[sort], depth[sort], ranks[sort]
        points_seg_tmp = points_seg_tmp[sort]

        kept2 = torch.ones(coor.shape[0], device=coor.device, dtype=torch.bool)
        kept2[1:] = (ranks[1:] != ranks[:-1])
        coor, depth, points_seg_tmp = coor[kept2], depth[kept2], points_seg_tmp[kept2]
        coor = coor.to(torch.long)
        depth_map[coor[:,1],coor[:,0]] = depth
        seg_map[coor[:,1],coor[:,0]] = (points_seg_tmp>=0).to(torch.int16)
        return depth_map, seg_map

    def __call__(self, results):
        points_lidar = results['points']
        points_seg = results['points_seg']
        imgs, rots, trans, intrins, post_rots, post_trans = results['img_inputs']
        depth_map_list = []
        seg_map_list = []
        for cid in range(rots.shape[0]):
            points_seg_tmp = torch.tensor(copy.deepcopy(points_seg))
            combine = rots[cid].matmul(torch.inverse(intrins[cid]))
            combine_inv = torch.inverse(combine)
            points_img = (points_lidar.tensor[:,:3] - trans[cid:cid+1,:]).matmul(combine_inv.T)
            points_img = torch.cat([points_img[:,:2]/points_img[:,2:3],
                                   points_img[:,2:3]], 1)
            points_img = points_img.matmul(post_rots[cid].T)+post_trans[cid:cid+1,:]
            depth_map, seg_map = self.points2depthsegmap(points_img, points_seg_tmp, imgs.shape[2], imgs.shape[3])
            depth_map_list.append(depth_map)
            seg_map_list.append(seg_map)
        depth_map = torch.stack(depth_map_list)
        seg_map = torch.stack(seg_map_list)
        results['img_inputs'] = (imgs, rots, trans, intrins, post_rots, post_trans, depth_map, seg_map)
        return results
...
@PIPELINES.register_module()
class LoadMultiViewImageFromFiles_CRTFusion(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def __init__(self, data_config, is_train=False,
                 sequential=False, aligned=False, trans_only=True,
                 load_point_label=False, grid_config=None, downsample=16):
        self.is_train = is_train
        self.data_config = data_config
        self.normalize_img = torchvision.transforms.Compose((
            torchvision.transforms.ToTensor(),
    ...
    def img_transform(self, img, post_rot, post_tran,
        ...
        return img, post_rot, post_tran, post_mat
    ...
    def get_downsampled_gt_depth_and_semantic(self, gt_depths, gt_semantics):
        ...
        gt_semantics[gt_depths < self.grid_config['dbound'][0]] = 0
        gt_semantics[gt_depths > self.grid_config['dbound'][1]] = 0
        gt_depths[gt_depths < self.grid_config['dbound'][0]] = 0
        gt_depths[gt_depths > self.grid_config['dbound'][1]] = 0
        gt_semantic_depths = gt_depths * gt_semantics

        H, W = gt_semantics.shape
        
        H_feat, W_feat = H // self.downsample, W // self.downsample
        d = torch.arange(*self.grid_config['dbound'], dtype=torch.float)\
        ...
        gt_semantics = gt_semantics.permute(0, 2, 4, 1, 3).contiguous()
        gt_semantics = gt_semantics.view(
            -1, self.downsample * self.downsample)
        gt_semantics = torch.max(gt_semantics, dim=-1).values
        gt_semantics = gt_semantics.view(H // self.downsample, W // self.downsample)
        ...
        gt_depths = gt_depths.view(H // self.downsample, W // self.downsample)
        gt_depths = (gt_depths -
                     (self.grid_config['dbound'][0] - self.grid_config['dbound'][2])) / self.grid_config['dbound'][2]
        ...
        gt_semantic_depths = gt_semantic_depths.permute(0, 2, 4, 1, 3).contiguous()
        gt_semantic_depths = gt_semantic_depths.view(
            -1, self.downsample * self.downsample)
        gt_semantic_depths =  torch.where(gt_semantic_depths == 0.0,
                                    1e5 * torch.ones_like(gt_semantic_depths),
                                    gt_semantic_depths)
        gt_semantic_depths = (gt_semantic_depths - (self.grid_config['dbound'][0] - 
                            self.grid_config['dbound'][2])) / self.grid_config['dbound'][2] 
    ...
    def point_label_transform(self, point_label, resize, resize_dims, crop, flip, rotate):
        ...
        gt_depths, gt_semantics = self.get_downsampled_gt_depth_and_semantic(torch.Tensor(depth_map), torch.Tensor(semantic_map))
        return gt_depths, gt_semantics
    ...
    def get_inputs(self,results, flip=None, scale=None):
        ...
        for cam in cams:
            ...
            resize, resize_dims, crop, flip, rotate = self.sample_augmentation(H=img.height,
                                                                               ...
                                                                               flip=flip,
                                                                               scale=scale)
            img, post_rot2, post_tran2, post_mat = self.img_transform(img, post_rot, post_tran,
                                                            resize=resize,
                                                            resize_dims=resize_dims,
                                                            crop=crop,
                                                            flip=flip,
            ...
            if self.load_point_label:
                ...
                point_label = np.load(point_filename, allow_pickle=True).astype(np.float64)[:4].T
            ...
            lidar2cam.append(lidar2cam_rt.T)
            viewpad = np.eye(4)
            viewpad[:intrin.shape[0], :intrin.shape[1]] = intrin
            
            new_cam = post_mat@viewpad
            cam_intrinsics.append(new_cam)
            lidar2img_rt = new_cam@lidar2cam_rt.T
            lidar2img_rts.append(torch.tensor(lidar2img_rt).unsqueeze(0))
            ...
            post_rots.append(post_rot)
            post_trans.append(post_tran)
        results['lidar2img'] = torch.cat(lidar2img_rts)
...
@PIPELINES.register_module()
class LoadPointsSegFromFile(object):
    """Load Points From File.
    ...
    def __call__(self, results):
        ...
        points = points.reshape(-1, self.load_dim)
        points = points[:, self.use_dim]
        ptseg_filename = results['ptseg_filename']
        points_seg = np.load(ptseg_filename)
        attribute_dims = None
...
@PIPELINES.register_module()
class LoadRadarPointsFromSampleSweeps(object):
    """Load points from multiple sweeps.
    ...
    def __call__(self, results):
        ...
        if self.rda_aug_conf is not None:
            ...
            radar_idx = np.arange(6)
        points_save = []
        for i in radar_idx:
            points_save.append(points[points[:, -1] == i]) ### radar sweep drop
        points_save.append(points[points[:, -1] == 10]) ### key radar points
        points = np.concatenate(points_save, axis=0)[:, :-1]
        
        num_pts, num_feats = points.shape
        if num_pts < self.max_radar_pts:
            choices = np.random.choice(num_pts, self.max_radar_pts)
            points = points[choices]
        
        points_class = get_points_type(self.coord_type)
        points = points_class(
            points, points_dim=points.shape[-1], attribute_dims=None)
        # x y z rcs vx_comp vy_comp dyn_prop id vx vy is_quality_valid ambig_state x_rms y_rms invalid_state pdh0 vx_rms vy_rms
        results['radar'] = points[:, [0,1,2,5,8,9,3,4,6,7,10,11,12,13,14,15,16,17]]
        return results
