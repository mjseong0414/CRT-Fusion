...
class RandomFlip3D(RandomFlip):
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal', flip_mat=None):
        ...
        for key in input_dict['bbox3d_fields']:
            if 'points' in input_dict:
                ...
                if 'radar' in input_dict:
                    input_dict['radar'].flip_radar(direction)
            else:
    ...
    def update_transform(self, input_dict):
        ...
        # for lidar2img matrix
        rot_mat = input_dict['pcd_rotation']
        scale_bda = input_dict['pcd_scale_factor']
        scale_mat = torch.Tensor([[scale_bda, 0, 0], [0, scale_bda, 0],
                                  [0, 0, scale_bda]])
        flip_mat = input_dict['flip_mat']
        rot_mat_1 = rot_mat @ (scale_mat @ flip_mat)
        bda_mat = torch.zeros(4, 4)
        bda_mat[3, 3] = 1
        bda_mat[:3, :3] = rot_mat_1
        l2c = np.array(input_dict['lidar2cam']).copy()
        cam_intrinsics = input_dict['cam_intrinsics'].copy()
        l2c_ = l2c @ bda_mat.numpy()
        input_dict['lidar2img'] = [cam_intrinsics[0]@l2c_[0],cam_intrinsics[1]@l2c_[1],cam_intrinsics[2]@l2c_[2],
                                   cam_intrinsics[3]@l2c_[3],cam_intrinsics[4]@l2c_[4],cam_intrinsics[5]@l2c_[5]]
    ...
    def __call__(self, input_dict):
        ...
        if input_dict['pcd_horizontal_flip']:
            flip_mat = self.random_flip_data_3d(input_dict, 'horizontal', flip_mat)
            input_dict['transformation_3d_flow'].extend(['HF'])
        if input_dict['pcd_vertical_flip']:
            flip_mat = self.random_flip_data_3d(input_dict, 'vertical', flip_mat)
            input_dict['transformation_3d_flow'].extend(['VF'])
        input_dict['flip_mat'] = flip_mat
...
class GlobalRotScaleTrans(object):
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        for key in input_dict['bbox3d_fields']:
            if len(input_dict[key].tensor) != 0:
                ...
                # for radar
                if 'radar' in input_dict:
                    input_dict['radar'].rotate_radar(-noise_rotation)
    ...
    def _scale_bbox_points(self, input_dict):
        ...
        # for radar
        if 'radar' in input_dict:
            _ = input_dict['radar'].scale_radar(scale)
