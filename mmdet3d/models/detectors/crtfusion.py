...
class CRTFusion(BEVDet):
    def __init__(self, 
                 ...
                 motion_th=1.0,
                 **kwargs):
        super(CRTFusion, self).__init__(**kwargs)
    ...
    def forward_test(self, points=None, img_metas=None, img_inputs=None, radar=None, **kwargs):
        ...
        if num_augs > 1:
            ...
            new_radar=[]
            a,b,c,d,e,f=[],[],[],[],[],[]
            for inputs, metas, rad in zip(img_inputs, img_metas, radar):
                a.append(inputs[0])
                b.append(inputs[1])
                c.append(inputs[2])
                d.append(inputs[3])
                e.append(inputs[4])
                f.append(inputs[5])
                new_img_metas.append(metas[0])
                new_radar.append(rad[0])
            a=torch.cat(a, dim=0)
            b=torch.cat(b, dim=0)
            c=torch.cat(c, dim=0)
            d=torch.cat(d, dim=0)
        ...
        if num_augs==1:
            ...
            return self.aug_test(points, new_img_metas, new_img_inputs, new_radar, **kwargs)
    ...
    def aug_test(self, points, img_metas, img=None, radar=None, rescale=False):
        """Test function with augmentaiton."""
        img_feats, _, _, _, _ = self.extract_img_feat(radar, img, img_metas)
        bbox_list = dict()
        pts_bbox = self.aug_test_pts(img_feats[0], img_metas, rescale=rescale)
    ...
    def aug_test_pts(self, feats, img_metas, rescale=False):
        ...
        for x, img_meta in zip(feats, img_metas):
            outs = self.pts_bbox_head([x.unsqueeze(0)])
